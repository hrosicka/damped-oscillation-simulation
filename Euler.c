#include <stdio.h>
#include <stdlib.h> // Added for exit() function, which is often used in professional code.

/**
 * @file main.c
 * @brief Simulates a damped harmonic oscillator using the explicit Euler method.
 *
 * This program models the motion of a mass-spring-damper system. It calculates the position
 * and velocity of the mass over time and writes the results to a file. The simulation
 * uses a simple numerical integration technique called the explicit Euler method.
 */
int main(void) {
    ...
    while (current_time <= total_time) {
        ...
        // --- 4b. NUMERICAL INTEGRATION (EXPLICIT EULER METHOD) ---
        // This method approximates the system's state at the next time step.
        // A temporary variable is needed to correctly update position and velocity.
        double old_velocity = velocity;

        // Update velocity using the calculated acceleration.
        velocity += acceleration * delta_t;
        
        // Update position using the velocity from the *previous* time step.
        // This is a key characteristic of the explicit Euler method.
        position += old_velocity * delta_t;
    ...
    printf("Simulation complete. Data has been saved to 'damped_oscillation_Euler.txt'.\n");
